...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const { telemetry, missionMode, missionTimeSeconds, isPaused, simulationSpeed, incidents } = data;
  ...
  return (
    <header className="border-b border-white/10 bg-[#0d1017]/90 backdrop-blur-xl sticky top-0 z-40 shadow-xl shrink-0">
      {/* Top Bar: Brand, Team attribution, and Primary Telemetry HUD */}
      ...
      <div className="px-3 sm:px-4 py-1.5 flex flex-wrap items-center justify-between gap-2 bg-[#090b10]">
        ...
        {/* Tactical Controls & Drill Anomaly Menu */}
        <div className="flex items-center space-x-2">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1 rounded text-xs font-mono font-bold bg-gradient-to-r from-amber-500/30 via-orange-500/30 to-rose-500/30 text-amber-300 border border-amber-500/50 hover:border-amber-400 transition shadow-[0_0_12px_rgba(245,158,11,0.25)]"
              title="Inject survivor signature forward of drone to demonstrate REC-SVLP investigation cycle [A]"
            >
              <Flame className="w-3 h-3 text-amber-400" />
              <span>TRIGGER DRILL ANOMALY</span>
            </button>
          </div>
        </div>
      </div>
